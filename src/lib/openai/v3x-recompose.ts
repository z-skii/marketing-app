...
export type RecomposeKey = "hero" | "recreate" | "drive" | "loyalty" | "user-home" | "user-profile" | "business-home" | "business-loyalty";
...
export const RECOMPOSE_EXPERIENCES: Record<RecomposeKey, { name: string; anchor: string; asks: string }> = {
  ...
  // the app surfaces: the approved public language propagated, not a new direction
  "user-home": { name: "User Home", anchor: "/design-lab-v3/home", asks: "WHAT CAN I EARN FROM RIGHT NOW: Recreate, Story and Car as three visually distinct valuable objects with the money obvious, the media large and very little copy; details after tap. The approved recomposition language (spatial depth, selective physical glass, dark material stage, premium object treatment, the Recreate and Drive identities), not a feed of cards." },
  "user-profile": { name: "User Profile", anchor: "/design-lab-v3/profile", asks: "Identity, not settings: portrait, name, reputation, completed work, work and media, Instagram, vehicle; one Settings gear; shareable as tapmart.live/@username. The approved recomposition language." },
  "business-home": { name: "Business Home", anchor: "/design-lab-v3/business", asks: "WHO OR WHAT CAN GROW MY BUSINESS: creators as work plus personality plus a quick request; cars as vehicle plus zone plus location plus rate plus request in the approved Drive language. Not a CRM, a directory, a spreadsheet or a classified listing." },
  "business-loyalty": { name: "Business Loyalty Home", anchor: "/design-lab-v3/business/loyalty", asks: "The full approved Loyalty architecture kept; the main view visually communicates members, returns, rewards and campaign attribution without turning into an analytics dashboard, and the campaign to customer to return loop feels connected to the approved public Loyalty sequence." },
};
...
export async function runV3XRecomposeVerify(i: { key: RecomposeKey; shots: string[]; notes?: string | null }, o: V2Options = {}): Promise<{ data: Record<string, unknown> | null; usage: Usage[]; files: string[]; markdown: string }> {
  ...
  const instructions = [
    WHO,
    "This job is the V3 RECOMPOSITION FINAL VERIFICATION. The founder approved the visual direction on the four public experiences; these captures (and recording strips, when the surface has motion) are the finished state, recorded from the final code, of either the finishing pass on a public experience or the propagation of that approved language to an app surface (User Home, User Profile, Business Home, Business Loyalty Home). Judge an app surface as a working product surface that must carry the approved language, with the same fixtures, flows and truth rules as before. You are verifying the actual finished state, not directing further polish. Return READY when a professional product designer would ship this experience as shown, or exactly one SPECIFIC BLOCKER: a defect visible in these pixels that a professional would not ship, named by where, what and why. A blocker is a defect (a collision, a clipped or hidden element, an unreadable state, a false claim, a broken handoff), not a preference. Everything else goes in notes and is not a required change. Score the experience out of 10 on the same scale as your comparison, so the founder can read before and after.",
    NO_SLOP,
    "=== YOUR RECOMPOSITION DIRECTION ===", direction, "=== END ===",
    "=== THE RECOMPOSITION BRIEF ===", m.brief, "=== END ===",
    `=== THE SIX QUESTIONS ===\n${SIX_QUESTIONS.map((q, n) => `${n + 1}. ${q}`).join("\n")}\n=== END ===`,
