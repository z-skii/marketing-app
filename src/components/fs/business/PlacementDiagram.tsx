...
type Region = { x: number; y: number; w: number; h: number } | { points: string };
const REGIONS: Record<ZoneKey, Region[]> = {
  ...
  rear_window: [{ points: "800,182 845,204 928,318 852,318" }],
  ...
  partial_wrap: [{ x: 345, y: 340, w: 695, h: 205 }, { points: "800,182 845,204 928,318 852,318" }],
  full_wrap: [{ x: 160, y: 190, w: 890, h: 360 }],
};

export function PlacementDiagram({ zones, width = 160, label, dark = false, muted = false, style }: {
  zones: readonly string[]; width?: number; label?: string; dark?: boolean; muted?: boolean; style?: React.CSSProperties;
}) {
  const h = Math.round((width * 2) / 3);
  const ink = dark ? "#F6F8FB" : "#151B23";
  const ground = dark ? "#101820" : "#FFFFFF";
  const line = dark ? "#3A4756" : "#CBD3DD";
  const wash = muted ? "#E7E7E2" : dark ? "rgba(175, 200, 255, 0.28)" : "#E7EDFF";
  const accent = muted ? "#526171" : dark ? "#AFC8FF" : "#2450E8";
  ...
  return (
    <svg width={width} height={h} viewBox="0 0 1200 800" role="img" aria-label={label ?? (chosen.length ? `Placement diagram: ${chosen.length} placement${chosen.length === 1 ? "" : "s"} highlighted` : "Placement diagram: no placement chosen")} style={{ display: "block", maxWidth: "100%", height: "auto", ...style }}>
      ...
      {chosen.flatMap((z) => REGIONS[z].map((r, i) => (
        "points" in r
          ? <polygon key={`${z}-${i}`} points={r.points} fill={wash} stroke={accent} strokeWidth="8" strokeLinejoin="round" />
