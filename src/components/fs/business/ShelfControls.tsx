 ...
 * Native trackpad, touch and keyboard scrolling keep working; reduced
 * motion jumps.
 */
export function ShelfControls({ target, label }: { target: string; label: string }) {
  ...
  return (
    <span className="fs-shelf-controls">
      <button type="button" className="fs-icon-btn fs-shelf-btn" aria-label={`Previous ${label}`} disabled={state.start} onClick={() => move(-1)}><CaretLeft size={20} /></button>
      <button type="button" className="fs-icon-btn fs-shelf-btn" aria-label={`Next ${label}`} disabled={state.end} onClick={() => move(1)}><CaretRight size={20} /></button>
    </span>
  );
}
