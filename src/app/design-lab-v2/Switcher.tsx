 ...
 */
export function Switcher({ current, className = "identity", compact = false, avatar = false }: { current: "Personal" | "Business"; className?: string; compact?: boolean; avatar?: boolean }) {
  const rows = [
  ...
  return (
    <Sheet title="Use TapMart as" variant="menu" triggerClass={className} triggerLabel={`${me.mode}: ${me.name}. Switch`} trigger={compact ? <>{avatar && <Avatar src={me.avatar} name={me.name} initials={me.initials} size={28} />}<span className="who">{me.mode === "Personal" ? "Personal" : me.name}</span><CaretDown size={16} aria-hidden style={{ color: "var(--v2-muted)" }} /></> : <IdentityLabel name={me.name} mode={me.mode} avatar={me.avatar} initials={me.initials} />}>
      {rows.map((r) => (
