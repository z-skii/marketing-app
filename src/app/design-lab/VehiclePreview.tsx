 ...
 * plane that ends at the assembly's lower right. With a real, permitted
...
export function VehiclePreview({ vehicle, style }: { vehicle: { label: string; year: number; body: string; color: string; city: string; photo: string; modelUrl: string | null }; style?: CSSProperties }) {
  return (
    <div style={style}>
      <div className="vehicle-assembly" style={{ display: "grid", gridTemplateColumns: "184px minmax(0, 1fr)", gap: 12, alignItems: "start" }}>
        ...
        <div style={{ marginTop: "var(--tm-shift-phone)", background: "var(--tm-surface)", minHeight: 124, padding: "12px 12px 0" }}>
