 ...
 * actions; never redrawn marketing UI. Every amount is a fixture.
 */

/** Recreate: graphite task block stepping down into the cobalt conditional-pay ledge. */
export function RecreateCommitment({ width = 198, height = 284, ledge = 112, detail = false, href = "/design-lab/user-home#recreate" }: { width?: number; height?: number; ledge?: number; detail?: boolean; href?: string }) {
  const r = opportunities.recreate;
  return (
    <div className="on-dark" style={{ width, minHeight: height, display: "flex", flexDirection: "column", background: "var(--tm-graphite)", color: "var(--tm-on-dark)" }}>
      <div style={{ padding: detail ? 20 : 12, flex: 1 }}>
        ...
        {detail && (
          <>
            <p className="t-meta" style={{ color: "var(--tm-muted-dark)", margin: "16px 0 0" }}>{r.spots} spots · Apply by {r.deadline}</p>
            <Link href={href} className="btn btn-secondary" style={{ marginTop: 16 }}>View work <ArrowRight size={18} aria-hidden /></Link>
          </>
        )}
      </div>
      <div style={{ background: "var(--tm-accent)", padding: detail ? 20 : 12, minHeight: ledge, display: "flex", flexDirection: "column", justifyContent: "flex-end" }}>
...
/** Story: the commitment beside the intact supplied creative; a 3x64 cobalt edge beside the amount only. */
export function StoryCommitment({ width = 198, detail = false, href = "/design-lab/user-home#story" }: { width?: number; detail?: boolean; href?: string }) {
  ...
  return (
    <div style={{ width, display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ marginTop: 8, borderLeft: "3px solid var(--tm-accent)", paddingLeft: 12, minHeight: 64, display: "flex", alignItems: "center" }}><Money cents={s.payCents} per={s.basis} /></div>
      <p className="t-task" style={{ margin: "12px 0 0" }}>{s.title}</p>
      <p className="t-meta" style={{ margin: "4px 0 0" }}>{s.business}</p>
      <p className="t-meta" style={{ margin: "4px 0 0" }}>{s.minFollowers.toLocaleString()}+ followers</p>
      {detail && (
        <>
          <p className="t-meta" style={{ margin: "16px 0 0" }}>{s.spots} spots · Apply by {s.deadline}</p>
          <p className="t-meta" style={{ margin: "4px 0 0" }}>Work state · <span className="status neutral">Open</span></p>
...
/** Car: the monthly campaign commitment in ink, attached below the physical scene. */
export function CarCommitment({ width = 358, inset = 12, detail = false, href = "/design-lab/user-home#car" }: { width?: number; inset?: number; detail?: boolean; href?: string }) {
  const c = opportunities.car;
  return (
    <div style={{ width: width - inset, marginLeft: inset, background: "var(--tm-surface)", minHeight: 96, padding: detail ? 20 : 12, display: "grid", gridTemplateColumns: "128px minmax(0, 1fr)", gap: 12, alignItems: "start" }}>
      ...
      <div>
        ...
        {detail && (
          <>
            <p className="t-meta" style={{ margin: "8px 0 0" }}>{c.zones} · {c.durationDays} days</p>
            <p className="t-meta" style={{ margin: 0 }}>Vehicle required</p>
