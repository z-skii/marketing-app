 ...
 * Resume motion control sits beside the compact lab context on every
 * surface. Pause freezes presentation timelines and CSS animations
 * without resetting their position (data-motion="paused" on the root);
 * product state changes still render immediately. The system
 * prefers-reduced-motion preference is authoritative: sequences render
 * their ordered states in document flow and Resume cannot re-enable
 * spatial motion.
 */
...
/** The compact context line: the Design Lab entrance (scenario control) and the motion control. */
export function LabStrip({ label, className = "", right }: { label?: string; className?: string; right?: ReactNode }) {
  return (
    <div className={`x-labstrip ${className}`}>
      <LabEntrance label={label} />
      <PauseControl />
