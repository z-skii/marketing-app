...
import { Preview } from "../../design-lab-v2/Preview";
import { Viewer } from "../../design-lab-v2/Viewer";
import { Sheet } from "../../design-lab-v2/Sheet";
import { Edge, Money } from "../../design-lab-v2/parts";
...
/** The V3 vehicle record: Eli’s car is listed in Round Rock, where Eli lives; the discovery city stays Austin. */
const businessCar = { ...v2Car, city: "Round Rock" };
import { LoyaltyStrip } from "./LoyaltyStrip";
import { LabEntrance } from "../LabControl";

/**
 * V3 Business Home discovery: the V2 marketplace as built (WHO OR WHAT
 * CAN MARKET MY BUSINESS) with the one shallow Loyalty strip the director
 * placed after the complete car action band on phone and across the
 * content width below the featured spread on desktop, in every filter.
 * Copied from the V2 Lab so /design-lab-v2 stays untouched. A person is
...
export function Discovery() {
  ...
  if (tab === "for-you") {
    phoneList.push(<PersonObject key="maya" p={maya} />, car && <CarObject key="car" />, <LoyaltyStrip key="loy" />, <PersonObject key="nora" p={nora} />, more ? <PersonObject key="eli" p={eli} /> : <button key="more" type="button" className="link t-action biz-more" onClick={() => setMore(true)}>More</button>);
  } else if (tab === "cars") {
    phoneList.push(car ? <CarObject key="car" /> : <Empty key="e" text="No cars match this view." />, <LoyaltyStrip key="loy" />);
  } else {
    ...
    phoneList.splice(car ? 2 : 1, 0, <LoyaltyStrip key="loy" />);
  }
  return (
    <>
      ...
      <div className={`biz-spread biz-${tab}`} data-more={more ? "true" : "false"}>
        ...
        <div className="biz-desk">
          {tab === "cars" ? (<>{car ? <CarObject /> : <Empty text="No cars match this view." />}<LoyaltyStrip /></>) : (
            <>
