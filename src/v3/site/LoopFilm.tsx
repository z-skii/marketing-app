...
import { liveProgram } from "../examples";
import { LateImg } from "../LateImg";
import { M } from "../media";
import { FilmControls, Scene, openEase, tl, useFilm, type Tracks, type Viewport } from "../Film";
...
function card(visits: number, ready: boolean): CardData {
  return { design: liveProgram.card, program: liveProgram, firstName: "Sara", memberId: "LD-001", code: "LMQ-7K2P-SARA", qr: "tapmart-example-member:loopday:q7n4k9r2m6t8", progress: ready ? 5 : visits, ready: ready ? 1 : 0, state: ready ? "ready" : "collecting", publicSubset: true };
}
...
function Stage({ staticAt, platform, setPlatform }: { staticAt: number | null; platform: "apple" | "google"; setPlatform: (p: "apple" | "google") => void }) {
  ...
  const comingSoon = <span className="t-fact x-open-preview">Coming soon</span>;
  const WalletOptions = (
    <Sheet title="Wallet options" variant="menu" triggerClass="link link-plain t-action x-lp-wallet" trigger="Wallet options">
      <div className="tabs" role="tablist" aria-label="Platform" style={{ marginTop: 8 }}>
        <button type="button" role="tab" aria-selected={platform === "apple"} onClick={() => setPlatform("apple")}>Apple Wallet example</button>
        <button type="button" role="tab" aria-selected={platform === "google"} onClick={() => setPlatform("google")}>Google Wallet example</button>
      </div>
    ...
    <div className="x-scene-stage x-loop-stage" ref={stage} data-beat={beat} data-act={ACT(beat)}>
      <div className="x-field">
        ...
        <div className="x-obj x-lp2-source" data-film="source">
          ...
          <span className="x-lp2-name">
            ...
            <Sheet title="Jasmine" variant="menu" triggerClass="link link-plain x-lp2-name-btn" trigger="Jasmine">
              ...
              <p className="t-fact" style={{ marginTop: 8 }}>Jasmine is an example creator, not a real account. Loyalty is coming soon: nothing here is issued to a Wallet.</p>
            </Sheet>
            </span>
          </span>
        ...
        <div className="x-obj x-paper x-lp2-signup" data-film="signup">
          ...
          <span className="t-fact">Agreed to the example program terms and privacy notice.</span>
      ...
      <div className="x-stage-head"><h2 className="x-film-title" id="loyalty-h">Loyalty</h2><span className="x-lp2-act"><span className="t-object">{ACT(beat)}</span><span className="t-note">Example sequence · Coming soon</span></span></div>
      <div className="x-stage-foot">
        {staticAt === null ? <FilmControls film={film} beats={LOOP_BEATS} playLabel="Play sequence">{WalletOptions}{comingSoon}</FilmControls> : <span className="x-frame-label">{beat + 1} of {LOOP_BEATS.length}<span aria-hidden> · </span>{LOOP_BEATS[beat].label}</span>}
      </div>
    </div>
  );
}
