...
import { homeOpportunities, profile, money } from "../examples";
import { M } from "../media";
...
export function Recreate() {
  ...
  const Actions = <Link href="/sign-up" className="link t-action x-open-preview">Start earning</Link>;
  if (reduced) {
    return (
...
export function Post() {
  ...
  const handoff = (
    <div className="x-po-handoff">
      ...
      <span className="t-object">{story.title}</span>
      <Link href="/sign-up" className="btn btn-primary">Start earning</Link>
...
// ------------------------------------------------------------- Get paid
/** Get paid: a recorded example whose gross and fee follow the product's live platform fee; the payout minimum is the live setting too. */
export function GetPaid({ feePct, minimumPayoutCents }: { feePct: number; minimumPayoutCents: number }) {
  const gross = Math.round(latte.netCents / (1 - feePct / 100));
  const fee = gross - latte.netCents;
  return (
    <Chapter id="get-paid" verb="Get paid">
      <div className="x-gp">
        ...
        <div className="x-gp-ledger paper">
          ...
          <span className="t-fact">Gross {money(gross, { cents: true })}<span aria-hidden> · </span>Fee {money(fee, { cents: true })}</span>
          <span className="t-object">Approval earns.</span>
          <span className="t-object">Payout is separate.</span>
          <Sheet title="Payout details" triggerClass="link t-action x-gp-payout" trigger="Payout details">
            <dl className="facts" style={{ marginTop: 8 }}>
              <div><dt>Payout minimum</dt><dd>{money(minimumPayoutCents, { cents: true })}</dd></div>
              <div><dt>Platform fee</dt><dd>{feePct}% of gross</dd></div>
              <div><dt>Payout fee</dt><dd>Not available</dd></div>
            </dl>
            <p className="t-body" style={{ marginTop: 16 }}>Approval creates earnings, not a completed payout. A recorded example.</p>
          </Sheet>
        </div>
      </div>
    </Chapter>
