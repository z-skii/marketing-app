...
import { homeOpportunities, businessPeople, money } from "../examples";
  ...
  return (
    <div className={`x-scene-stage x-earn-stage${inHero ? " x-dark x-earn-hero" : ""}`} ref={stage} data-beat={beat} data-audience={aud} data-stage-tone={inHero ? "dark" : "inspection"}>
      <div className="x-field">
        ...
        <span className="x-obj x-plane x-plane-r6 x-earn-portrait" data-film="portrait"><img src={M.portraitMaya(480)} alt={`${maya.name}, example creator`} width={480} height={600} decoding="async" fetchPriority="low" />{business && <span className="x-tag x-obj-tag">{maya.name}</span>}</span>
        ...
        <Link href={preview} className="x-obj x-plane x-plane-r6 x-plane-fore x-earn-ref" data-film="ref" aria-label={`Start earning: ${recreate.title}, ${recreate.business}, example campaign`}>
        ...
        <div className="x-obj x-paper x-earn-terms" data-film="terms">
          {business ? (
            <><span className="t-object">{maya.name}</span><span className="x-earn-terms-actions"><span className="t-fact">Example creator</span><Link href="/sign-up" className="link t-action">Request</Link></span></>
          ) : (
        ...
        <div className="x-obj x-paper x-earn-sub" data-film="submission">
          <span className="t-fact">Submission example</span>
          ...
          <span className="t-fact">An example. Nothing is submitted.</span>
        </div>
        ...
        {inHero && staticAt === null ? (
          <div className="x-hero-actions x-reveal" key="hero-actions">
            {business ? (
              ...
              <span className="x-hero-actions-r"><button type="button" className="btn btn-primary" onClick={() => film.go(1)}>Explore earning</button><Link href={preview} className="link t-action">Start earning</Link></span>
            )}
          </div>
        ) : staticAt === null ? (
