...
import type { CardDesign, Program } from "../examples";
...
/** The payload every member QR carries: that member's own example card address, so the counter scanner and the card agree. */
export const cardUrl = (origin: string, code: string) => `${origin}/business/loyalty?card=${code}`;
...
export function PlatformLabel({ platform, above = true }: { platform: "apple" | "google"; above?: boolean }) {
  return above ? <span className="t-fact-ink wc-platform-label">{platform === "apple" ? "Apple Wallet example · Coming soon" : "Google Wallet example · Coming soon"}</span> : <span className="t-note wc-platform-note">No pass is issued.</span>;
}
