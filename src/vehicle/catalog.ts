...
const AI_FACTS = {
  ...
  meshy: { triangles: 206616, meshes: 1, textures: "4 WebP at 2048 px (base colour, normal, metallic roughness, emissive); Draco geometry", fileBytes: 2471536, technique: "THREE.DecalGeometry projected onto the generated body triangles inside the door box" },
...
export const AI_G80_CANDIDATES: Record<string, VehicleModel> = {
  ...
  "ai-g80-meshy": aiG80("ai-g80-meshy", "/vehicles/ai-g80-meshy.glb", "Meshy multi image to 3D, four photographs", Math.PI, AI_G80_ZONES, AI_FACTS.meshy),
...
/** The candidate on /labs/ai-g80 by default: the most detailed one, both ends true to the photographs. */
export const AI_G80: VehicleModel = AI_G80_CANDIDATES["ai-g80-tripo"];

export const AI_G80_REPORT: AiReport = {
  geometry: "A real triangle mesh in a GLB file, rendered by three.js. Not an image, not a splat, not a downloaded BMW model. Generated by a multi view image to 3D model from four photographs of one real car; the mesh, its textures and every surface the photographs did not show were produced by that model.",
  provider: "Tripo H3.1 multiview to 3D (detailed geometry and texture, PBR), run through TapMart's Higgsfield account on 2026-09-23. Two other providers ran on the same inputs: Meshy multi image to 3D (?v=ai-g80-meshy) and Hunyuan3D v3 (?v=ai-g80-hunyuan2).",
  inputs: "Four photographs of one Alpine White M3 Competition G80 in a car park: front, rear, rear three quarter (driver side) and front three quarter (passenger side). No straight side view exists in the series. Public dimensions 4,794 x 1,903 x 1,433 mm and 2,857 mm wheelbase scale the mesh to real size.",
  timeAndCost: "About 5 minutes from submission to GLB. 18 Higgsfield credits for this generation; 116 credits across the five generations and five cutouts of the whole experiment, from the account's existing plan. No new key or purchase.",
  file: "GLB, 2.16 MB on the web (1,900,141 triangles and 57.8 MB as generated; simplified to 475,008 triangles with Draco compression). One mesh, one PBR material, positions, normals and one UV set. Wheels, glass, lights and body are one surface with no separate parts.",
  textures: "Base colour, occlusion roughness metallic and normal maps, 4096 px JPEG as generated, 2048 px WebP on the web.",
  dimensions: "The bounding box is 1.85 long for every 1 wide and 2.38 long for every 1 tall. The real car is 2.32 (over mirrors) and 3.35. Scaled to the real length the mesh would be 2.59 m wide and 2.01 m tall: the model made the car about a quarter too short. It is shown at real length, so it reads as a taller, stubbier M3.",
  decal: "The Demo Coffee Co. creative is projected with THREE.DecalGeometry onto the generated triangles inside a driver front door box, so it follows the surface and stays attached while the car turns. The door box is placed by hand on the fitted mesh; the generated door has no separate geometry and its window edge is noisy.",
  accurate: [
    "Front: the vertical kidney grille, the slim headlights, the three part lower bumper and the splitter read as an M3 Competition G80 at a glance.",
    "Rear: the light shape, the diffuser and the four exhaust tips match the photographs.",
    "Details the photographs showed: the fender gills, the M mirrors, the roof spoiler lip, the wheel design and the red calipers.",
    "Stance and ride height look right from the front and the rear three quarter views.",
  ],
  wrong: [
    "Proportions: the car is about 25 percent too short for its width and height, so the side view shows a tall, short sedan with a short wheelbase. This is the biggest error.",
    "Side surfaces: the window glass and the door area are wrinkled where the photographs gave no straight side view; the door shut lines are faint and partly invented.",
    "Hood and roof carry texture noise from the car park lighting; the panoramic roof edge is smeared.",
    "Wheels are one surface with the body (no separate wheel geometry) and the tyres are thin in places.",
    "Meshy got the proportions right (within 1 percent of the width over mirrors) but its surfaces and textures are muddy; Hunyuan3D has the cleanest surfaces but invented a two door side with no door lines and is also too short.",
  ],
  references: [
