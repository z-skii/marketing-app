...
export default async function QuickClosePage({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  const [day, { data: locSettings }, { data: categories }] = await Promise.all([
    ...
    supabase.from("location_settings").select("starting_cash, require_closing_checklist").eq("location_id", location.id).maybeSingle(),
  ...
  return (
    <>
      ...
      <QuickCloseForm
        key={`${location.id}:${date}:${day.report?.updated_at ?? "new"}`}
